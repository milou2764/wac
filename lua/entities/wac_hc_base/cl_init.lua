...
function ENT:receiveInput(name, value, seat)
	...
	elseif name == "Camera" then
		local player = LocalPlayer()
		if value > 0.5 then
			player:GetVehicle().useCamera = not player:GetVehicle().useCamera
		end
	end
end
...
function ENT:Think()
	...
	if not self:GetNWBool("locked") then
		...
		if not IsValid(e) then e = LocalPlayer() end
		local pos = e:GetPos()
...
function ENT:getPassenger(seat)
	if not IsValid(self:GetSwitcher()) then return end
...
function ENT:attachmentThink()
	if not self.weaponAttachments or not self.Camera then return end
	local camAng = self:getCameraAngles()
	if not camAng then return end
...
function ENT:DrawHUD(k,p)
	if not self.Seats or not self.Seats[k] or p:GetViewEntity()~=p then return end
...
function ENT:Draw()
	...
	if self.engineRpm > 0.2 and self.SmokePos then
		if !self.lastHeatDrawn or self.lastHeatDrawn < CurTime()-0.1 then
            if not self.Emitter or not self.Emitter:IsValid() then return end	-- sometimes it isnt valid for some reason
